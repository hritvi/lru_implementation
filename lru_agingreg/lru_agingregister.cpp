...
{
    int min_index = 0;
    int min = aging_regs[min_index];
    for(int i = 0; i < num_frames; i++) {
        if(aging_regs[i] < min) {
            min_index = i;
            min = aging_regs[min_index];
        }
    }

    frames[min_index] = new_page;
    aging_regs[min_index] += pow(2, history_to_look);
...
void print_frame_state(int frames[], int num_frames)
{
    ...
    for (int i = 0; i < num_frames; i+https://github.com/abhijeet219/uKnow/pulls+) {
        fprintf(fout, "    %d\n", frames[i]);
    }
}
